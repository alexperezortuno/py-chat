...
def receive_messages(page):
    """Receive messages from server."""
    while True:
        try:
            encrypted_message = client_socket.recv(4096).decode('utf-8')
            command: str = messages_income(json.loads(encrypted_message))
            logger.debug(f"Message: {command}")

            if command == 'CHANNELS':
                channel_list.controls.clear()
                for channel in json.loads(encrypted_message)['channels']:
                    channel_list.controls.append(ft.Text(channel))
                page.update()
            if command == 'MESSAGE':
                # Desencriptar mensaje (aquí suponemos que AES ya se comparte entre clientes)
                message = encrypted_message  # Ajustar según el cifrado
                ...
                page.update()
            if command == 'CHANNEL_CREATED' and encrypted_message['username'] == global_params['username']:
...
def main(page: ft.Page):
    ...
    def register_user(user_name: ft.TextField):
        ...
        if connection_established:
            ...
            client_socket.send(json.dumps({'type': 'REGISTER', 'user_name': global_params['username']}).encode('utf-8'))
            page.go(route_channel)
    ...
    def route_change(route: str):
        ...
        if connection_established:
            client_socket.send(json.dumps({'type': 'CHANNELS'}).encode('utf-8'))
                        ...
                        [
                            ft.Column(
                                ...
                                controls=[
                                    ...
                                    ft.Container(
                                        ft.ElevatedButton("Save", on_click=lambda e: register_user(input_field)),
                                        adaptive=True,
                                        alignment=ft.alignment.center
                                    )],
                            ),
        ...
        threading.Thread(target=receive_messages, args=(page), daemon=True).start()
